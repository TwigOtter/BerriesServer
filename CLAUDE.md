...
- **`berries_bot/`** — Config/assets only. `personality.txt` is the character prompt loaded by `shared/ask_berries.py`. `lore/facts.md` holds curated character facts and is **injected verbatim into every personality prompt** by `LoreProvider` (edit it and the change is live — no reindex, no restart); `lore/server-rules.md` is read on demand by the `get_server_rules()` tool. Lore is excluded from vector search so it never competes with transcript chunks. See `berries_bot/lore/README.md` for why retrieval didn't work here.
...
- `retrieval.py` — RAG retrieval stage: query rewriting → multi-query vector search → assist-model reranking (with abstain) → retrieval logging. Searches transcripts/summaries/Discord only; `source: "lore"` is filtered out in `chroma_client.query_chroma_multi`.
