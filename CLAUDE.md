...
- **`berries_bot/`** — Config/assets only. `personality.txt` is the character prompt loaded by `shared/ask_berries.py`. `lore/facts.md` holds curated character facts, currently reached via **ordinary vector search** alongside transcript chunks (`LoreProvider` exists but is not wired into any pipeline — see the interim note in `berries_bot/lore/README.md`); `lore/server-rules.md` is read on demand by the `get_server_rules()` tool.
...
- `retrieval.py` — RAG retrieval stage: query rewriting → multi-query vector search → assist-model reranking (with abstain) → retrieval logging. Searches transcripts/summaries/Discord/lore — all sources compete in one pool.
