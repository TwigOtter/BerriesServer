...
- `WINDOW_ENABLED=true`, `WINDOW_TOKEN_LIMIT=100` — pre-rerank window selection (`shared/windowing.py`): shrink each candidate chunk to its best ~150-token window. Load-bearing when `RERANK_ENABLED=true` — disabling it puts `RERANK_CANDIDATES` full chunks in one rerank prompt, overflowing the 4096-token vLLM context
